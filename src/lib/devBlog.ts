...
export const BLOG_ENTRIES: BlogEntry[] = [
  ...
  },
  {
    id: 13,
    date: '2026-09-11',
    content: `COMBINED MULTI-TRAIL REPORTS — Cover several trails in one outing? You can now roll them into a single report instead of one email per trail. In Settings → Data Logger → Multi-Trail Report, pick "Combined" (the default stays "Separate", the way it worked before). In Combined mode, changing the Trail dropdown no longer ends your report — it totals up the trail you're leaving, closes out that trail's section, and keeps right on going, so the finished report reads trail-by-trail with each section clearly separated and its own tallies. On screen, the People/Dogs/Trees counters reset to zero for each new trail so you always see the current trail's count, with a small "All trails" running total tucked beneath.\n\nAlso: the first choice in the Trail dropdown is now clearer — "Non-PWV or Off Trail" — for when you're logging somewhere that isn't a PWV trail (it still hides the On Trail light and the distance readouts). — KDG (v1.49.0)`,
  },
]
